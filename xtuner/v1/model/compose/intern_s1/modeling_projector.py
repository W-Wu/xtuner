...
from .intern_s1_config import InternS1ProjectorConfig,InternS1TimeSeriesProjectorConfig
...
class InternS1TimeSeriesProjector(BaseModel):
    config: InternS1TimeSeriesProjectorConfig

    def __init__(self, config: InternS1TimeSeriesProjectorConfig):
        super().__init__()
        self.layer_norm = nn.LayerNorm(config.time_series_hidden_size * int(1 / config.downsample_ratio) ** 2)
        self.linear_1 = nn.Linear(
            config.time_series_hidden_size * int(1 / config.downsample_ratio) ** 2, config.text_hidden_size
        )
        self.act = get_act_fn(config.hidden_act)
        self.linear_2 = nn.Linear(config.text_hidden_size, config.text_hidden_size)

        self._hf_prefix = "model.time_series_projector."
        self._init_load_spec()
